...
import { ShareBadge, ShareStatus } from '@/components/embed-popup/share-banner';
...
export function Trigger({ error = null, popupOpen, onToggle }: TriggerProps) {
  ...
  const sharing = isScreenShareEnabled && !popupOpen;
      ...
      <AnimatePresence>
        ...
        >
          ...
          >
            <AnimatePresence>
              {!popupOpen && (
                ...
                >
                  <PhoneIcon size={20} weight="fill" className="size-5 text-[var(--color-bg1)]" />
                </motion.div>
              )}
          ...
          {/* The trigger is fixed to the viewport, so this is the one sharing indicator that
            survives both a closed panel and any scroll position. Visual only: see
            ShareStatus below for why the words are not in here. */}
          {sharing && <ShareBadge />}
        </AnimatedButton>
      </AnimatePresence>

      {/* Outside the button on purpose: text inside it would become the button's
          accessible name. */}
      {sharing && <ShareStatus />}
    </>
  );
